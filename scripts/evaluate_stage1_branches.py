...
class RegionAccumulator:
    ...
    def update(self, pred: Tensor, target: Tensor, mask: Tensor) -> dict[str, float]:
        ...
        if count <= 1.0e-6:
            return {"mae": math.nan, "rmse": math.nan, "psnr": math.nan, "bias": math.nan, "sam_deg": math.nan, "region_frac": 0.0}
        diff = pred - target
        ...
        return self.current(region_frac=float(mask.mean().item()))

    def current(self, region_frac: float | None = None) -> dict[str, float]:
        if self.count <= 1.0e-6:
            return {"mae": math.nan, "rmse": math.nan, "psnr": math.nan, "bias": math.nan, "sam_deg": math.nan, "region_frac": region_frac or 0.0}
        mae = self.abs_sum / self.count
        rmse = math.sqrt(self.sq_sum / self.count)
        psnr = 20.0 * math.log10(1.0 / max(rmse, 1.0e-12))
        ...
        return {
            ...
            "sam_deg": sam,
            "region_frac": region_frac if region_frac is not None else math.nan,
            "count": self.count,
        }
...
def main() -> None:
    ...
    acc: dict[str, RegionAccumulator] = defaultdict(RegionAccumulator)
    bucket_acc: dict[str, RegionAccumulator] = defaultdict(RegionAccumulator)
